...
const QUESTION = {
  ...
  questionNumber: QUIZ_SEQUENCE[3], // "Pergunta 29 de 30"
  totalQuestions: 30,
  label: "Infrações e Penalidades",
  errorRate: "82%",
  text: "Durante uma blitz, o condutor se recusa a realizar o teste do bafômetro (etilômetro). Qual é a consequência prevista na legislação de trânsito?",
  options: [
    {
      id: "a",
      text: "Nenhuma penalidade, pois a Constituição garante que ninguém é obrigado a produzir prova contra si mesmo.",
    },
    {
      id: "b",
      text: "Infração gravíssima, multa multiplicada por 10 e suspensão do direito de dirigir por 12 meses.",
    },
    {
      id: "c",
      text: "Infração grave, multa simples e apreensão da CNH por 6 meses até apresentação de recurso.",
    },
    {
      id: "d",
      text: "Detenção imediata e encaminhamento à delegacia para realização de exame de sangue compulsório.",
    },
  ],
  correctId: "b",
};
...
export default function StepPergunta04() {
          ...
          {!confirmed ? (
            ...
            >
              ...
              >
                ...
                >
                  {isCorrect ? "✓ Impressionante. Essa era a mais difícil." : "✗ Caiu na armadilha constitucional."}
                </p>
                <p className="text-xs leading-relaxed text-[oklch(0.4_0.01_250)]">
                  {isCorrect
                    ? "Recusar o bafômetro tem exatamente a mesma punição que dirigir bêbado: gravíssima, multa x10, 12 meses de suspensão (Art. 165-A, CTB). Poucos candidatos sabem disso."
                    : "O princípio de não produzir prova contra si existe na Constituição. Mas o Art. 165-A do CTB é específico: recusar o teste é infração gravíssima, com multa multiplicada por 10 e suspensão por 12 meses. A mesma punição de quem dirige alcoolizado."}
                </p>
              </div>

              {/* O href aponta para uma página que será criada no futuro (o step 3) */}
              <Link href="/step-06-detran" className="w-full py-4 rounded-2xl bg-[oklch(0.25_0.01_250)] text-white text-sm font-semibold flex items-center justify-center gap-2 active:scale-[0.98] transition-transform duration-150 shadow-[0_4px_16px_oklch(0.15_0.01_250/20%)]">
                Será que eu passaria hoje?
                <ArrowRight size={16} />
              </Link>
            </motion.div>
          )}
